"use client";

import { useTransition } from "react";
import { deleteSchedule } from "@/lib/actions";

export function DeleteScheduleButton({ scheduleId, label }: { scheduleId: string; label: string }) {
  const [pending, startTransition] = useTransition();
  return (
    <button
      disabled={pending}
      onClick={() => {
        if (
          window.confirm(
            `Delete "${label}"?\n\nUpcoming chores from this schedule will be removed. Completed history and points are kept.`
          )
        ) {
          startTransition(() => deleteSchedule(scheduleId));
        }
      }}
      className="border-line rounded-lg border px-3 py-1.5 text-sm font-medium text-red-700 disabled:opacity-60"
    >
      {pending ? "…" : "Delete"}
    </button>
  );
}
